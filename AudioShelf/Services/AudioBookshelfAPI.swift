...
class AudioBookshelfAPI {
    ...
    func getPodcasts(libraryId: String) async throws -> [Podcast] {
        ...
        return podcastsResponse.results
    }
    ...
    func getEpisodes(podcastId: String) async throws -> [Episode] {
        ...
        guard let url = URL(string: "\(serverURL)/api/podcasts/\(podcastId)/episodes") else {
            throw APIError.invalidURL
        }
        ...
        let sortedEpisodes = episodesResponse.episodes.sorted { episode1, episode2 in
    ...
    func getCoverImageURL(for podcast: Podcast) -> URL? {
        guard let serverURL = serverURL,
              let coverPath = podcast.coverImageURL else {
            return nil
        }

        return URL(string: "\(serverURL)\(coverPath)")
    }
}
